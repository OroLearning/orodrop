  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none opacity-[0.2] dark:opacity-[0.3] z-0 select-none">
      {elements.map((el, i) => (
        ...
        >
          {el.type === 'word' && (
            <span className="text-oroYellow font-black text-[8px] md:text-[9px] tracking-[0.35em] uppercase whitespace-nowrap">
              {el.content}
            </span>
          )}
          {el.type === 'icon' && renderIcon(el.icon!, `${el.size} text-oroYellow`)}
          {el.type === 'streak' && (
            <div className={`${el.length} h-[1px] bg-gradient-to-r from-transparent via-oroYellow/60 to-transparent rotate-[-35deg]`}></div>
          )}
        </div>
      ))}
    </div>
      ...
      <div className="relative w-full max-w-lg bg-white dark:bg-darkCard rounded-[48px] md:rounded-[56px] shadow-heavy p-8 md:p-12 border border-slate-100/50 dark:border-darkBorder z-10 transition-colors duration-300 overflow-hidden">
         ...
         <div className="relative z-10 flex justify-center mb-10">
            <div className="w-20 h-20 md:w-24 md:h-24 bg-oroYellow rounded-[28px] md:rounded-[32px] flex items-center justify-center shadow-lg shadow-oroYellow/20">
               <svg className="w-10 h-10 md:w-12 md:h-12 text-slate-900" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <path d="M6 16.326A7 7 0 1 1 15.71 8h1.79a4.5 4.5 0 0 1 .5 8.973" />
                <path d="m13 11-4 6h6l-4 6" />
               </svg>
            </div>
         ...
         <div className="relative z-10 mt-10 flex flex-col gap-4">
            ...
            <button 
               ...
               disabled={isLoading}
               className={`relative w-full h-16 rounded-3xl font-black text-xs uppercase tracking-[0.2em] transition-all duration-500 shadow-xl flex items-center justify-center gap-3 overflow-hidden ${isLoading ? 'bg-oroYellow text-slate-900' : 'bg-slate-900 dark:bg-oroYellow text-white dark:text-slate-900 hover:bg-oroYellow hover:text-slate-900'}`}
            >
               {isLoading && (
               ...
               {isLoading ? (
                 <>
                   <div className="relative w-6 h-6 flex flex-col items-center">
                      <svg className="w-6 h-4 text-slate-900 mb-[-4px]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 13a4 4 0 0 1 0 8H7a5 5 0 0 1 0-10 5 5 0 0 1 8 2z" />
                      </svg>
                      <div className="flex gap-1">
                        <div className="w-0.5 h-1.5 bg-slate-900 rain-drop"></div>
                        <div className="w-0.5 h-1.5 bg-slate-900 rain-drop"></div>
                        <div className="w-0.5 h-1.5 bg-slate-900 rain-drop"></div>
                      </div>
                   </div>
